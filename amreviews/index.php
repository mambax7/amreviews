...
if(!isset($_REQUEST['op'])) {
	...
	*/
	if (isset($_GET['id'])) {
		$sqlquery = "WHERE cat_parentid = '" . intval($_GET['id']) . "' AND cat_showme='1' ";
		//echo $sqlquery;
	} else {
		...
		*/
		if (isset($_GET['id'])) {
			...
			*/
			...
			if ($gperm_handler->checkRight('Category Permission', intval($_GET['id']), $groups, $xoopsModule->getVar('mid'))) {
			...
			*/
			...
			" WHERE (date_publish='0' OR ". time() ." > date_publish) AND (date_end='0' OR ". time() ." < date_end) AND validated='1' AND showme='1' AND catid='" . intval($_GET['id']) . "' ORDER BY date DESC");
			...
			if ($xoopsDB->getRowsNum($result) > 0) {
				while($myrow = $xoopsDB->fetchArray($result)) {
					...
					*/
					if ($myrow['image_align'] == "L") { $amr_reviews['image_align'] = "left"; }
						else { $amr_reviews['image_align'] = "right"; }
